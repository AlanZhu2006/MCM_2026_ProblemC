...
def create_professional_zip():
    """创建专业模板ZIP文件"""
    print("=" * 70)
    print("创建专业模板ZIP文件")
    print("=" * 70)
    
    template_dir = Path('latex_professional')
    zip_path = Path('latex_professional.zip')
    
    if not template_dir.exists():
        print(f"错误: {template_dir} 文件夹不存在")
        print("请先运行 create_professional_template.py")
        return
    ...
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        # 添加主文件
        if (template_dir / 'main.tex').exists():
            zipf.write(template_dir / 'main.tex', 'main.tex')
            print("  main.tex")
        
        # 添加README
        if (template_dir / 'README.md').exists():
            zipf.write(template_dir / 'README.md', 'README.md')
            print("  README.md")
        ...
        # 添加所有图表文件
        visualizations_dir = template_dir / 'visualizations'
